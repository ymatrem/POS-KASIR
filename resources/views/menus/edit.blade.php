...
@section('page-subtitle')
    <p class="text-sm text-gray-500">Ubah informasi menu</p>
@endsection
...
@section('content')
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <form action="{{ route('menus.update', $menu->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" id="name" name="name" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 @error('name') border-red-500 @enderror" value="{{ old('name', $menu->name) }}" required>
                    @error('name')
                ...
                <div class="mb-6">
                    ...
                    <textarea id="description" name="description" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 @error('description') border-red-500 @enderror">{{ old('description', $menu->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="number" id="price" name="price" step="0.01" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 @error('price') border-red-500 @enderror" value="{{ old('price', $menu->price) }}" required>
                        @error('price')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" id="stock" name="stock" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 @error('stock') border-red-500 @enderror" value="{{ old('stock', $menu->stock) }}" min="0" required>
                        @error('stock')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mb-6">
                    ...
                    <div id="imagePreview" class="mt-4">
                        <img id="previewImage" src="{{ $menu->image_url ?? '' }}" alt="Preview" class="max-w-xs h-auto rounded-lg shadow" {{ $menu->image_url ? '' : 'style="display: none;"' }}>
                        <button type="button" id="removeImage" class="mt-2 bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 transition" {{ $menu->image_url ? '' : 'style="display: none;"' }}>
                            <i class="fas fa-trash mr-2"></i> Hapus Gambar
                        </button>
                    </div>

                    <!-- Hidden input to store image URL -->
                    <input type="hidden" id="imageUrl" name="image_url" value="{{ old('image_url', $menu->image_url) }}">
    ...
    <script>
        ...
        // Update drop zone display based on current image
